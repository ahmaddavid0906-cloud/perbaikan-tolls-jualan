...
`;
  ...
  async function callGeminiWithFallback(
    ...
    const gatewayPayload = {
      ...
      endpoint: `/api/${inferredTool.toLowerCase().replace(/[^a-z0-9]/g, '-')}`
    };
...
function sanitizeCaptionsAndHashtags(text: string): string {
  ...
  app.post('/api/generate-prompt', async (req, res) => {
    ...
    try {
      ...
      if (!userSelectedModel || userSelectedModel === 'gemini-3.6-flash' || userSelectedModel === 'gemini-3.1-pro-preview') {
        userSelectedModel = (analysisMode === 'deep') ? 'gemini-3.1-pro-preview' : 'gemini-3.6-flash';
      }
      ...
      const analysisDepthGuide = analysisMode === 'deep'
        ? 'MODE ANALISIS: DEEP ANALYSIS (Mendalam & Sinematik Detail). Analisis setiap micro-action, transkrip dialog ultra-akurat kata per kata, orientasi pencahayaan tiga titik (key/fill/rim), grading warna, dan sinematografi lensa kamera.'
        : 'MODE ANALISIS: FAST ANALYSIS (Cepat & Responsif). Ekstrak aksi esensial, pacing adegan, dialog kunci, dan estetika visual secara presisi dan efisien.';

      // SEO Caption & Hashtag Guide
      const seoCaptionGuide = sourceCaption && sourceCaption.trim()
        ? `\n### INSTRUKSI CAPTION & HASHTAG SEO VIRAL (SANGAT PENTING - MAKSIMAL 5 HASHTAG):
Terdapat CAPTION ASLI dari sumber video TikTok input:
"""${sourceCaption.trim()}"""

Tugas Anda: Analisis adegan visual, objek, tindakan, serta suara/dialog video ini secara mendalam.
Buatlah CAPTION VIRAL SEO ALGORITMA TIKTOK / REELS / SHORTS FYP yang 100% SANGAT RELEVAN DENGAN KONTEN VIDEO:
1. RELEVANSI VISUAL MUTLAK: Caption wajib 100% selaras dengan topik, aksi visual, objek, dan emosi di dalam video.
2. INTEGRASI SHORT & LONG QUERIES: Caption WAJIB secara aktif memuat banyak Short Search Queries (kata kunci pencarian singkat) dan Long-tail Search Queries (frasa pencarian spesifik/panjang) yang paling sering dicari audiens di pencarian sosial media.
3. STRUKTUR COPYWRITING: Hook menggigit di kalimat pertama, storytelling / benefit persuasif yang kuat, dan Call to Action (CTA) yang jelas.
4. ATURAN HASHTAG MUTLAK (MAKSIMAL 5 HASHTAG):
   - Tulis MAKSIMAL 5 HASHTAG (paling sedikit 3, PALING BANYAK 5 HASHTAG). DILARANG KERAS menghasilkan lebih dari 5 hashtag!
   - Seluruh 5 hashtag WAJIB 100% relevan dengan konteks adegan video dan isi caption (kombinasi broad tag, niche tag spesifik, dan search query tag).`
        : `\n### INSTRUKSI CAPTION & HASHTAG SEO VIRAL (SANGAT PENTING - MAKSIMAL 5 HASHTAG):
Berdasarkan adegan visual, objek, tindakan, dan dialog/suara video ini:
1. RELEVANSI VISUAL MUTLAK: Buat CAPTION SEO FYP TIKTOK / REELS / SHORTS yang 100% selaras dan sangat relevan dengan adegan video.
2. SHORT & LONG SEARCH QUERIES: Caption WAJIB mengandung banyak Short Search Queries (kata kunci pencarian singkat) dan Long-tail Search Queries (frasa pencarian spesifik/panjang) yang paling sering dicari pengguna di search bar.
3. STRUKTUR COPYWRITING: Hook menggigit di kalimat pertama, penjelas benefit/storytelling persuasif, dan Call to Action (CTA) yang jelas.
4. HASHTAG MUTLAK (MAKSIMAL 5 HASHTAG):
   - Tulis MAKSIMAL 5 HASHTAG (antara 3 sampai 5 HASHTAG, DILARANG LEBIH DARI 5 HASHTAG).
   - Semua hashtag HARUS 100% relevan dengan konteks visual video dan isi caption yang dibuat.`;
      ...
      if (segmentDuration !== 'auto') {
        ...
        promptText = `Anda adalah AI Video Analyst & Shot Breakdown Specialist kelas dunia.

Tugas: Analisis video ini dengan sangat teliti, lalu pecah menjadi segmen-segmen dengan durasi masing-masing sekitar ${sec} detik.

ATURAN OUTPUT WAJIB (ikuti 100%, jangan dilanggar):

1. JANGAN gunakan format [Style], [Environment], [Camera], [Actions], [Lighting], dll.
2. JANGAN buat prompt panjang berbahasa Inggris.
3. WAJIB pakai format timeline Bahasa Indonesia seperti contoh di bawah.
4. Setiap segmen HARUS punya tepat 3 bagian: Visual, Aksi, dan Subteks.
5. Visual harus sangat detail (lokasi, orang, pakaian, ekspresi, objek, pencahayaan, sudut kamera).
6. Aksi menjelaskan gerakan yang terjadi di segmen tersebut.
7. Subteks adalah teks overlay / voice-over / makna yang muncul atau tersirat (buat natural seperti caption TikTok jika tidak ada teks asli).
8. Timestamp harus akurat (contoh: 0–2 detik, 2–3,8 detik, 3,8–5,5 detik).
9. Jangan menambah pembukaan, penutup, atau penjelasan di luar format.
10. Langsung mulai dari segmen pertama.

CONTOH FORMAT YANG WAJIB DIIKUTI:

0–2 detik
Visual: Di dalam toko perlengkapan rumah yang terang, seekor kucing oranye besar terbaring di lantai. Tiga wanita muda (satu mengenakan hijab hitam dan celana jeans, satu mengenakan kaus krem dan celana hitam, satu lagi mengenakan hijab biru dan kemeja kotak-kotak sambil membawa keranjang merah) sedang berkumpul dan tampak terkejut.
Aksi: Tiga wanita menunduk menatap "kucing" di lantai, menunjukkan rasa penasaran dan terkejut yang besar.
Subteks: Lihat tuh, orang-orang sampai berhenti gara-gara keset ini.

2–3,8 detik
Visual: Jarak menengah-dekat, wanita berkaus krem berlutut dan mengulurkan tangan untuk mengelus punggung kucing, sementara temannya di samping berdecak kagum dan tersenyum.
Aksi: Wanita mengulurkan tangan mengelus bulu kucing, ekspresi temannya terlihat sangat hidup.
Subteks: Awalnya dikira kucing, ternyata keset kamar mandi.

3,8–5,5 detik
Visual: Pegawai toko dan pembeli bersama-sama mengangkat keset berbentuk kucing dari lantai, memperlihatkan wujud aslinya sebagai keset kamar mandi.
Aksi: Pegawai toko dan pembeli bersama-sama mengangkat keset yang lembut, menampilkan kelenturan dan bentuknya.
Subteks: Pas dipegang, bulunya tebal dan lembut banget.

Lanjutkan pola yang sama sampai akhir video.

Mode analisis: ${analysisMode === 'deep' ? 'Deep Analysis — buat Visual dan Aksi sangat kaya detail.' : 'Fast Analysis — tetap detail tapi lebih ringkas.'}`;
      } else {
        promptText = `Anda adalah AI Video Analyst & Shot Breakdown Specialist kelas dunia.

Tugas: Analisis video ini dengan sangat teliti, lalu pecah video berdasarkan perubahan adegan alami (bukan durasi tetap).
...
Mode analisis: ${analysisMode === 'deep' ? 'Deep Analysis — buat Visual dan Aksi sangat kaya detail.' : 'Fast Analysis — tetap detail tapi lebih ringkas.'}`;
      ...
      const videoPromptSystemInstruction = `You are an elite video shot-breakdown analyst. You MUST follow the user instruction format 100%. Output ONLY in Indonesian timeline format:
...
- Do NOT use [Style], [Environment], [Camera], [Lighting], [Actions], or any English bracket tags.
- Do NOT output Master Prompt code blocks.
- Do NOT add introductions, conclusions, or extra sections before the first timestamp.
- Every segment MUST have exactly three fields: Visual, Aksi, Subteks.
- Timestamps must be accurate (e.g. 0–2 detik, 2–3,8 detik).
- Start directly from the first segment.
- Language of Visual, Aksi, and Subteks must be Bahasa Indonesia.`;
...
${promptText}`
      ...
      // Sanitize captions & hashtags strictly according to TikTok modern semantic search indexing rules (Max 5, anti-spam)
      finalPromptText = sanitizeCaptionsAndHashtags(finalPromptText);
...
- **💡 Rekomendasi Prompt Generator**: [Saran model gambar terbaik seperti Midjourney v6.1, Flux.1, atau Ideogram v2].`;
      ...
      const isUltraMode = Boolean(
        ...
        req.body?.isUltra ||
        req.body?.mode === 'ultra' ||
        req.body?.analysisMode === 'deep' ||
        req.body?.analysisMode === 'ultra' ||
        (typeof photoStyle === 'string' && photoStyle.toLowerCase().includes('ultra'))
      );
      const photoTargetTier = isUltraMode ? 'flagship' : 'tier2';
  ...
  async function fetchTikTokShopProduct(shopUrl: string): Promise<{
    ...
    try {
      if (!shopUrl || !shopUrl.includes('tiktok')) return null;

      // Coba pakai endpoint yang sudah ada di proyek jika ada (misalnya scrape / proxy)
      // Jika belum ada endpoint khusus shop, lakukan fetch sederhana ke URL
      // dan ambil text seadanya (fallback aman).

      const res = await fetch(shopUrl, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; SatsetBot/1.0)',
      ...
      if (contentType.includes('application/json')) {
        ...
        text = await res.text();
        // Ambil potongan relevan saja agar tidak terlalu panjang
        text = text
      ...
      // Ekstrak kasar nama & deskripsi dari text
      const nameMatch = text.match(/(?:product[_-]?name|title|nama produk)["'\s:]+([^"'\n]{5,80})/i);
      const priceMatch = text.match(/(?:price|harga)["'\s:]+([^"'\n]{2,30})/i);
      ...
      console.warn('[fetchTikTokShopProduct] gagal:', err);
      return null;
    }
  }
...
`.trim();
          logger.info('[Content Ideas] Data produk TikTok Shop berhasil diambil.');
        } else {
          productContext = `
...
ATURAN PENTING TENTANG PRODUK:
1. Jika DATA PRODUK di atas tersedia, WAJIB menjadikan produk ini sebagai objek utama di SEMUA ide.
2. Jangan mengarang fitur produk yang tidak ada di data.
3. Ganti objek / produk yang muncul di video viral dengan produk ini (mapping).
4. Hook, pain point, benefit, CTA, dan caption harus relevan dengan produk ini.
5. Pertahankan struktur, pacing, dan pola visual dari video viral (Stage 1), tapi isinya tentang produk di atas.
6. Jika data produk gagal diambil (hanya ada link), gunakan field Topik / Nama Produk yang diisi user.
` : `
CATATAN PRODUK:
Tidak ada link TikTok Shop. Gunakan field Topik / Nama Produk yang diisi user sebagai acuan produk.
`}
...
4. HASHTAG WAJIB MAKSIMAL 5 HASHTAG PER IDE & FOKUS SEO TIKTOK SEARCH ALGORITHM (ANTI-SPAM & HIGH RELEVANCE):
   ...
   - DILARANG MENGGUNAKAN ISTILAH SPAM SEPERTI "FYP", "racun TikTok", "for your page" DI DALAM CAPTION.
   - HASHTAG HARUS SANGAT SPESIFIK & TARGETED BERDASARKAN PRODUK/DETAIL VISUAL DI TAHAP 1 & REFERENSI JUDUL/HASHTAG TIKTOK USER (#1 & #2 Nama Produk/Bahan, #3 & #4 Detail Utama, #5 Niche Audience). Total MAKSIMAL 5 HASHTAG DENGAN RELEVANSI DUA/TIGA ARAH 100% PERSIS DENGAN KONTEN ADEGAN VIDEO.
...
# 🚀 ${totalIdeas} IDE KONTEN VIRAL & AEO ENGINE OPTIMIZED (${maxSecNum}s, ${expectedClipsCount} Klip)
...
`;
      ...
      // =========================================================================
      const hasIdeaStructure =
        Boolean(finalOutputText) &&
        (finalOutputText.includes('IDE KONTEN VIRAL') ||
          finalOutputText.includes('IDE 1:') ||
          finalOutputText.includes('IDE #1') ||
          /IDE\s+[1-9]/i.test(finalOutputText));
      const isOutputProblematic = !finalOutputText || finalOutputText.trim().length < 300 || !hasIdeaStructure;
...
BERIKUT HASIL ${totalIdeas} IDE KONTEN YANG DI-GENERATE TAHAP 2 (TERDETEKSI KURANG LENGKAP ATAU KURANG STRUKTUR):
"""
${finalOutputText}
"""

TUGAS VALIDASI:
Lakukan verifikasi cross-check singkat dan lengkapi formatnya.
- Pastikan menghasilkan ${totalIdeas} IDE KONTEN VIRAL lengkap dengan hook, visual action, dan voice-over grounded.
- Perbaiki kekurangan/ketidaksesuaian aksi atau produk agar sesuai fakta visual Tahap 1, lalu kembalikan teks utuh yang sudah rapi dan lengkap.`,
              },
            ],
          },
          config: {
            systemInstruction:
              "You are a factual consistency cross-checker and content structuring expert. Ensure generated copy is complete, structured, and does not contradict video visual evidence.",
          },
        };
        ...
        try {
          ...
          if (validatedResult?.text && validatedResult.text.trim().length >= 300) {
      ...
      // Sanitize captions & hashtags strictly according to TikTok modern semantic search indexing rules (Max 5, anti-spam)
      finalOutputText = sanitizeCaptionsAndHashtags(finalOutputText);
...
Hasilkan 1 paragraf padat berbahasa Inggris yang mendeskripsikan secara jelas ciri khas produk di gambar ini agar prompt video AI konsisten dan akurat.`;
          ...
          const anchorResult = await callGeminiWithFallback(
            model ? normalizeGeminiModel(model) : 'gemini-3.5-flash-lite',
            anchorPayload,
            customApiKey,
            clientAccessCode,
            'tier2',
...
\`\`\`text
[Style]: Bright, commercial e-commerce product video, clean and polished aesthetic.
[Environment]: Indoor studio, showroom or aesthetic setup with product displayed nicely in English.${identityAnchorDescription ? ` Featuring: ${identityAnchorDescription}` : ''}
...
[Transition / Editing]: Single continuous shot.
[Call to Action]: Recommending product or action if applicable for this clip segment.
[Negative Prompt]: flickering, flicker, strobing, morphing face, warping identity, inconsistent character design between frames, changing outfit/product color mid-clip, unstable lighting, jittery motion, texture popping, banding artifacts, blurry transition, extra fingers, deformed hands, distorted proportions, watermark, text glitch, double exposure ghosting, low resolution, oversaturated color shift${identityAnchorDescription ? ', deviation from reference product image colors/packaging' : ''}
\`\`\``);
        currentSec = nextSec;
      }
      const timestampTemplateText = timestampGuideList.join('\n');
...
- WAJIB FORMAT MARKDOWN: Setiap list poin (-) dan block code (\`\`\`text) HARUS menggunakan baris baru (newline). JANGAN menggabungkan teks per-segmen klip menjadi satu paragraf.
- LARANGAN KLAIM HARGA & OVERCLAIM: JANGAN PERNAH menyebutkan nominal harga spesifik (misal: "80 ribuan", "under 190k") di script, VO, atau teks layar. JANGAN melakukan overclaiming (klaim berlebihan yang menyesatkan atau melanggar aturan e-commerce). Gunakan bahasa persuasif yang aman seperti "harga promo", "diskon spesial", dll.
- CAPTION & HASHTAG SEO INDEXING: Anda WAJIB membuat caption pada bagian "Draft Caption TikTok Shop" yang SANGAT RELEVAN dengan produk/video, mengandung HOOK persuasif, dan dibatasi maksimal 5 hashtag saja (contoh: #A #B #C #D #E). DILARANG KERAS menggunakan kata atau hashtag spam seperti #fyp, #racuntiktok, #foryou, #viral, #trending, #beranda, "FYP", "racun tiktok", "for your page". Seluruh 5 hashtag HARUS 100% relevan dengan produk dan ide video ini. Jangan masukkan caption ke dalam teks Rincian Adegan Video.
...
${attachedFile.textContent || attachedFile.content || '(Teks berkas terlampir)'}`;
  ...
  app.post('/api/test-gemini-key', async (req, res) => {
    try {
      ...
      const aiInstance = getGeminiClient(apiKey.trim());
      const startTime = Date.now();
      ...
      if (response && response.text) {
        return res.json({ success: true, latency, model: 'gemini-2.5-flash' });
      }
