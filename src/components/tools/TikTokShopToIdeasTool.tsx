...
export default function TikTokShopToIdeasTool({
  ...
  const [selectedModel, setSelectedModel] = usePersistentState<string>('tts_selectedModel', '');
    ...
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 pb-12">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 sm:p-6 lg:p-7 shadow-xs space-y-4 sm:space-y-5">
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
          ...
          {/* CONFIGURATIONS ROW: DURATION & SEGMENT SPLIT & TARGET MODE */}
          <div className="pt-2 grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
            
            {/* TOTAL DURATION TEMPLATES (10, 20, 30, 40, 50, 60, 70 Detik) */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-[11px] sm:text-xs font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
                  <Clock className="w-3.5 h-3.5 text-amber-500" />
                  TOTAL DURATION
                </label>
                <span className="text-[11px] font-bold text-amber-700 bg-amber-50 border border-amber-200/80 px-2 py-0.5 rounded-lg shadow-2xs">
                  {totalDuration} Detik
                </span>
              </div>

              <div className="h-11 p-1 bg-slate-100 border border-slate-200 rounded-xl grid grid-cols-7 gap-1">
                {[10, 20, 30, 40, 50, 60, 70].map((sec) => (
                  <button
                    key={sec}
                    type="button"
                    onClick={() => setTotalDuration(String(sec))}
                    className={`rounded-lg text-xs font-bold transition-all flex items-center justify-center cursor-pointer ${
                      totalDuration === String(sec)
                        ? 'bg-[#7c3aed] text-white shadow-2xs font-extrabold ring-2 ring-[#7c3aed]/20'
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full h-11 px-3.5 rounded-xl bg-white border border-slate-200 focus:border-[#7c3aed] text-slate-900 text-xs font-medium focus:outline-none cursor-pointer transition-all"
              >
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full h-11 px-3.5 rounded-xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-[#7c3aed] text-slate-900 text-xs font-medium focus:outline-none cursor-pointer transition-all"
              >
          ...
          {/* ROW: BACKGROUND SOUND, TEXT OVERLAY & REFERENCE IMAGE */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
            
            {/* LEFT COLUMN: TOGGLES */}
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3.5 rounded-xl border border-slate-200/80 bg-slate-50/60">
                <div>
                  <p className="text-xs font-bold text-slate-900">Background Sound / Efek Suara</p>
                  <p className="text-[11px] text-slate-500">
                    Sertakan tag audio dalam prompt
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between p-3.5 rounded-xl border border-slate-200/80 bg-slate-50/60">
                <div>
                  <p className="text-xs font-bold text-slate-900">Text Overlay / Hook Teks</p>
                  <p className="text-[11px] text-slate-500">
                    Sertakan teks layar dalam prompt
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input 
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-800 uppercase tracking-wider block">
                Foto Produk Referensi (Cegah Flicker Identitas)
              </label>
              {!refImageFile ? (
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-3 text-center cursor-pointer transition-all flex flex-col items-center justify-center min-h-[104px] ${
                    isRefDragging ? 'border-[#7c3aed] bg-purple-50' : 'border-slate-200 bg-slate-50 hover:border-[#7c3aed] hover:bg-slate-100/80'
                  }`}
                >
                  <ImageIcon className="w-5 h-5 text-[#7c3aed] mb-1" />
                  <p className="text-[11px] font-bold text-slate-800">Unggah Foto Produk Referensi (Opsional)</p>
                  <p className="text-[10px] text-slate-500">Klik atau seret file gambar ke sini</p>
                  <input
                    type="file"
                    ref={refFileInputRef}
                    onChange={(e) => {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 pt-2">
            ...
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl w-full sm:w-auto">
              <button
                ...
                className={`flex-1 sm:flex-none py-2 px-3.5 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                  analysisMode === 'deep'
                    ? 'bg-[#7c3aed] text-white shadow-2xs font-extrabold'
                    : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                <Sparkles className="w-3.5 h-3.5" />
                <span>Deep Scrape + Reasoning</span>
              </button>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-900">Google Search Live Grounding</span>
                  <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-[#7c3aed] uppercase tracking-wider">
                    REAL-TIME
                  </span>
                </div>
                <p className="text-[11px] text-slate-500">
                  Ambil riset data pasar & kata kunci pencarian aktual
                </p>
              </div>
            </div>
          ...
          <div className="flex justify-end pt-2">
            <button
              ...
              className="w-full sm:w-auto px-7 py-3 rounded-xl bg-[#7c3aed] hover:bg-[#6d28d9] text-white font-bold text-xs sm:text-sm transition-all shadow-xs hover:shadow flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 min-h-[44px] active:scale-[0.98]"
            >
              {isProcessing ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin text-purple-200" />
                  <span>Analyzing Product & Generating Ideas...</span>
                </>
              ) : (
                <>
