...
export default function ContentIdeasTool({
  ...
  const handleGenerateIdeas = async () => {
    ...
    if (tiktokShopUrl.trim()) {
      ...
      const isLikelyTikTok = url.includes('tiktok.com') || url.includes('shop.tiktok');
      if (!isLikelyTikTok) {
        alert('Link TikTok Shop sepertinya tidak valid. Pastikan mengarah ke tiktok.com / shop.tiktok.com, atau kosongkan field ini.');
        return;
      }
    }
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="https://shop.tiktok.com/... atau link produk TikTok Shop"
                  className="w-full h-11 px-4 rounded-xl bg-white border border-slate-200 focus:border-[#5b50e5] focus:ring-2 focus:ring-[#5b50e5]/20 focus:outline-none text-slate-900 text-xs placeholder:text-slate-400 transition-all"
                />
                <p className="text-xs text-slate-500 mt-1">
                  Jika diisi, sistem akan mencoba mengambil data produk lalu menyesuaikan ide ke produk ini. Jika gagal, tetap memakai Topik / Nama Produk yang kamu isi.
                </p>
                {tiktokShopUrl.trim() && (
                  <p className="text-xs text-emerald-600 mt-1 font-medium">
                    Link TikTok Shop terisi — data produk akan dicoba diambil saat generate.
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="pt-4 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <button
                  ...
                  className={`rounded-lg text-xs font-bold transition-all flex items-center justify-center cursor-pointer ${
                    maxDuration === String(sec)
                      ? 'bg-[#5b50e5] text-white shadow-2xs font-extrabold ring-2 ring-[#5b50e5]/20'
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              className="w-full h-11 px-3.5 rounded-xl bg-white border border-slate-200 focus:border-cyan-500 text-slate-900 text-xs focus:outline-none cursor-pointer font-medium"
            >
          ...
          >
            ...
            <div className="space-y-2 w-full max-w-md">
              ...
              <div className="flex justify-between text-[11px] text-slate-500 pt-1 font-medium">
                ...
                <span>Tahap 2: {numIdeas} Ide + VO Natural</span>
              </div>
            </div>
          </motion.div>
        ) : rawResult ? (
