...
export default function PhotoPromptGeneratorTool({
  ...
  const handleFileSelection = (file: File) => {
    ...
    if (file.size > MAX_IMAGE_SIZE) {
      setError('Ukuran file gambar terlalu besar. Silakan gunakan file di bawah 30MB.');
      return;
    }
    setImageFile(file);
  ...
  const handleGeneratePhotoPromptWithText = async (
    ...
    const titleName = inputMode === 'image' ? (imageFile?.name || 'Foto Referensi') : (targetText.slice(0, 40) + '...');
    ...
    try {
      ...
      if (inputMode === 'image' && !overrideText) {
        if (!imageFile) {
        ...
        mimeType = 'text/plain';
        base64Data = btoa(unescape(encodeURIComponent(targetText)));
      ...
      learningSync.track('photo_prompt_generated', {
        ...
        photoStyle,
        aspectRatio,
        title: inputMode === 'image' ? imageFile?.name : targetText.slice(0, 30),
      });
      ...
      saveHistoryItem({
        ...
        data: {
          ...
          photoStyle,
          aspectRatio,
          sourceText: inputMode === 'text' ? targetText : undefined,
