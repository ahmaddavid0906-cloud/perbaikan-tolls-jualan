...
import { dbGetApiKeys, dbSaveApiKeys, dbAddApiKeyLog, dbGetModelPriorities } from '../db/dbService';
...
export class LLMGateway {
  ...
  public async getAvailableKeyCandidates(
    ...
    // 1. User custom keys from headers / body
    ...
    // 4. Admin Database Multi-Key Pool
    try {
      ...
      for (const k of activeAdminKeys) {
        poolCandidates.push({
          key: k.key,
          keyId: k.id || `admin_key_${maskApiKey(k.key)}`,
          source: 'admin_pool',
        });
      }
    } catch (e) {
  ...
  public async execute(options: LLMGatewayRequestOptions): Promise<LLMGatewayResponse> {
    ...
    const isImageTool = inferredTool.toLowerCase().includes('photo') || inferredTool.toLowerCase().includes('image');
    const isTtsTool = inferredTool.toLowerCase().includes('tts') || inferredTool.toLowerCase().includes('audio') || inferredTool.toLowerCase().includes('voice');
    const defaultHierarchy = isImageTool 
      ? GATEWAY_IMAGE_MODELS_HIERARCHY 
      : isTtsTool 
        ? GATEWAY_TTS_MODELS_HIERARCHY 
        : GATEWAY_MODELS_HIERARCHY;
    ...
    try {
      ...
      if (storedPriorities) {
        if (isImageTool && Array.isArray(storedPriorities.image) && storedPriorities.image.length > 0) {
    ...
    // If user explicitly chose a model, place it first in the hierarchy; otherwise start from Flagship/top priority
    let candidateModels: string[];
    if (options.isUserExplicitChoice && options.model) {
