...
const CACHE_FILE = path.join(process.cwd(), 'local_db_store.json');
...
export const initDbSeed = async () => {
  console.log('[DB] initDbSeed ready');
};
...
export const dbSaveAnnouncement = async (item: any) => {
  if (Array.isArray(item)) {
    for (const i of item) await safeSave(FIRESTORE_COLLECTIONS.ANNOUNCEMENTS || 'announcements', i);
  } else {
    await safeSave(FIRESTORE_COLLECTIONS.ANNOUNCEMENTS || 'announcements', item);
  }
};
export const dbDeleteAnnouncement = async (id: string) => safeDelete(FIRESTORE_COLLECTIONS.ANNOUNCEMENTS || 'announcements', id);
export const dbGetAnnouncements = async () => safeGet(FIRESTORE_COLLECTIONS.ANNOUNCEMENTS || 'announcements');

export const dbGetFormulas = async () => safeGet(FIRESTORE_COLLECTIONS.PROMPT_FORMULAS || 'promptFormulas');
export const dbSaveFormula = async (item: any) => {
  if (Array.isArray(item)) {
    for (const i of item) await safeSave(FIRESTORE_COLLECTIONS.PROMPT_FORMULAS || 'promptFormulas', i);
  } else {
    await safeSave(FIRESTORE_COLLECTIONS.PROMPT_FORMULAS || 'promptFormulas', item);
  }
};
export const dbDeleteFormula = async (id: string) => safeDelete(FIRESTORE_COLLECTIONS.PROMPT_FORMULAS || 'promptFormulas', id);

export const dbGetAffiliates = async () => safeGet(FIRESTORE_COLLECTIONS.AFFILIATES || 'affiliates');
export const dbSaveAffiliate = async (item: any) => {
  if (Array.isArray(item)) {
    for (const i of item) await safeSave(FIRESTORE_COLLECTIONS.AFFILIATES || 'affiliates', i);
  } else {
    await safeSave(FIRESTORE_COLLECTIONS.AFFILIATES || 'affiliates', item);
  }
};
export const dbDeleteAffiliate = async (id: string) => safeDelete(FIRESTORE_COLLECTIONS.AFFILIATES || 'affiliates', id);
...
export const dbGetLoginUiSettings = async (): Promise<any> => (await safeGetOne(FIRESTORE_COLLECTIONS.CONFIGS || 'configs', 'loginUi')) || {};
export const dbSaveLoginUiSettings = async (data: any) => safeSave(FIRESTORE_COLLECTIONS.CONFIGS || 'configs', { id: 'loginUi', ...data });

export const dbGetUserUiSettings = async (): Promise<any> => (await safeGetOne(FIRESTORE_COLLECTIONS.CONFIGS || 'configs', 'userUi')) || {};
export const dbSaveUserUiSettings = async (data: any) => safeSave(FIRESTORE_COLLECTIONS.CONFIGS || 'configs', { id: 'userUi', ...data });
